...
# Perimeter Interface Information
PERIM_IFACE="eth1"
PERIM_IP="192.168.2.1"
PERIM_NET="192.168.2.0/24"
PERIM_BCAST="192.168.2.255"
...
initfirewall(){
	...
	$IPT -P INPUT DROP # drop incoming traffic
	$IPT -P FORWARD DROP # drop passthrough traffic
	$IPT -P OUTPUT ACCEPT # accept outgoing traffic
...
initperimeter() {
	$IPT -A FORWARD -i $LOCAL_IFACE -o $PERIM_IFACE -j ACCEPT # forward from LAN to Perimeter
	$IPT -A FORWARD -i $PERIM_IFACE -o $LOCAL_IFACE -j ACCEPT # forward from Perimeter to LAN
	$IPT -A FORWARD -i $INET_IFACE -o $PERIM_IFACE -j ACCEPT # forward from Internet to Perimeter
	$IPT -A FORWARD -i $PERIM_IFACE -o $INET_IFACE -j ACCEPT # forward from Perimeter to Internet

	$IPT -A INPUT -i $PERIM_IFACE -j ACCEPT # accept input traffic from Perimeter
}
...
initvpn(){
	...
	$IPT -A FORWARD -i $PERIM_IFACE -o $VPN_IFACE -j ACCEPT
	$IPT -A FORWARD -i $VPN_IFACE -o $PERIM_IFACE -j ACCEPT	
...
dnstransfers() {
	...
	# Destination NAT
	$IPT -t nat -A PREROUTING -p udp -i $INET_IFACE -d $INET_ADDRESS --dport 53 -j DNAT --to-destination $PERIM_IP:53
	$IPT -t nat -A PREROUTING -p tcp -i $INET_IFACE -d $INET_ADDRESS --dport 53 -j DNAT --to-destination $PERIM_IP:53
}
...
sshtraffic() {
	...
	# Destination NAT
	$IPT -t nat -A PREROUTING -p tcp -i $INET_IFACE -d $INET_ADDRESS --dport 22 -j DNAT --to-destination $PERIM_IP:22
}

httptraffic() {
	#Destination NAT http & https traffic
	$IPT -t nat -A PREROUTING -p tcp -i $INET_IFACE -d $INET_ADDRESS -m multiport --dport 80,443 -j DNAT --to-destination $PERIM_IP
}

ftptraffic(){
	$IPT -t nat -A PREROUTING -p tcp -i $INET_IFACE -d $INET_ADDRESS --dport 21 -j DNAT --to-destination $PERIM_IP:21
...
case $1 in
	start)
		...
		initperimeter && echo " -> Perimeter network ( $PERIM_NET ) configured."
